...
void createDescriptorSetLayout(vk_context *vko) {
    ...
    VkDescriptorSetLayoutBinding bindings[4] = { uboLayoutBinding, imageLayoutBinding, sphereBufferLayoutBinding, triangleBufferLayoutBinding };
...
void createDescriptorPool(vk_context *vko) {
    VkDescriptorPoolSize poolSizes[4]; // adjust for however many uniforms you want
...
void createDescriptorSets(vk_context *vko) {
    ...
    for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        VkWriteDescriptorSet descriptorWrites[4] = {0}; // can add more descriptor sets if necessary
        ...
        descriptorWrites[3].dstBinding = 3; // binding = 2 in the shader
        descriptorWrites[3].dstArrayElement = 0;
        descriptorWrites[3].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
        descriptorWrites[3].descriptorCount = 1;
        descriptorWrites[3].pBufferInfo = &triangleBufferInfo;

        vkUpdateDescriptorSets(vko->device, 4, descriptorWrites, 0, NULL);
    }
}
